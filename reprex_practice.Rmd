...
unique(mack_creek_vertebrates$SECTION)

```

```{r}
subset <- mack_creek_vertebrates %>% 
  clean_names() %>% 
  select(year:sampledate) %>% 
  filter( section == "CC") %>% slice_head(n = 5)
  
#datapasta::df_paste(subset)
 library(tidyverse)
library(janitor)
subset <- data.frame(
 ...
 subset %>% mutate(across(where(is.character, tolower))) %>% 
  drop_na(species) %>% 
  group_by(species) %>% 
  summarize(mean_length_cm = mean(length1, na.rm = TRUE),
            sd_length_cm = sd(length1, na.rm = TRUE)) %>% 
